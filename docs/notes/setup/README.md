...
## 1. Product Metadata Setup
The file [../../about.json](../../about.json) contains information about Chart Finder. This top-level metadata file drives all other files with the product name, author, and more. Thus the *first step* is to review it (if using this repo as a template for a new app, this file would of course change).

Once updated - `about.json` changes will automatically propagate everywhere else.
...
## 3. Environment Hydration (`.local`)
1. Run `make setup-dev-env` (top-level Makefile) which wraps `scripts/setup-dev-env.sh`. The script will prompt for the values gathered above (AWS account, `CF_LOCAL_ENV_ID`, `CF_LOCAL_AWS_ARTIFACT_BUCKET`, etc.) and hydrate:
   - `.local/local.env`
   - Hydrated infra templates under `.local/infra/**`
   - Cached metadata in `.local/state/**`
2. If you need to rebuild specific sections, use the `CF_ENV_VARS_OPTION_*` flags described in <a href="../environment-variables.md" target="_blank" rel="noopener">docs/notes/environment-variables.md</a>.

## 4. Initial Stack Management
- **Stack creation**  
...
## 6. Frontend Deployment Placeholder
