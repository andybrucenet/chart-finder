...
the_frontend_version_artifacts_spec_file="$the_frontend_version_artifacts_root_dir/docs/api/chart-finder-openapi-v1.json"
...
frontend_version_artifacts_generate_tmp() {
  ...
  if ! FRONTEND_VERSION_ARTIFACTS_BASE_URI="${CF_LOCAL_BASE_URI:-}" \
    FRONTEND_VERSION_ARTIFACTS_SPEC_PATH="$the_frontend_version_artifacts_spec_file" \
...
spec_path = os.environ.get("FRONTEND_VERSION_ARTIFACTS_SPEC_PATH", "")
spec_version = ""
spec_build = ""
if spec_path and os.path.exists(spec_path):
    try:
        with open(spec_path, "r", encoding="utf-8") as spec_handle:
            spec_data = json.load(spec_handle)
        info = spec_data.get("info", {})
        spec_version = info.get("x-chartfinder-backend-version", "")
        spec_build = info.get("x-chartfinder-backend-build-number", "")
    except Exception:
        spec_version = ""
        spec_build = ""
...
company_snake = to_snake(company)
product_snake = to_snake(product)
...
values = {
    ...
    "companySnake": company_snake,
    "productSnake": product_snake,
    "backendApiVersion": spec_version,
    "backendApiBuildNumber": spec_build,
}
...
export const VersionInfo = {{
  ...
  companySnake: `{companySnake}`,
  productSnake: `{productSnake}`,
  backendApiVersion: `{backendApiVersion}`,
  backendApiBuildNumber: `{backendApiBuildNumber}`,
}} as const;
"""
