...
"$the_aws_logout_root_dir/scripts/cf-run-cmd.sh" aws sts get-caller-identity --no-cli-pager >"$the_aws_logout_wrk_path" 2>&1
the_rc=$?
if [ $the_rc -eq 0 ] ; then
  if [ x"$AWS_LOGOUT_OPTION_QUIET" = x1 ] ; then
    "$the_aws_logout_root_dir/scripts/cf-run-cmd.sh" aws sso logout --no-cli-pager >"$the_aws_logout_wrk_path" 2>&1
    the_rc=$?
    [ $the_rc -ne 0 ] && cat "$the_aws_logout_wrk_path"
  else
    echo 'LOGGING_OUT'
    "$the_aws_logout_root_dir/scripts/cf-run-cmd.sh" aws sso logout --no-cli-pager
    the_rc=$?
  fi
else
